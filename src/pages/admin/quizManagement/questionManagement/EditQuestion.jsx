...
import { getEditQuestion, updateQuestion } from "../../../../api/adminDashboardApi"; // Adjust import path as needed
...
function EditQuestion() {
    ...
    const fetchQuestionDetails = async () => {
        try {
            setFetching(true);
            console.log("Fetching question details for quizId:", quizId, "and quesId:", quesId);
            const response = await getEditQuestion(quizId, quesId);
            ...
            setQuestionData({
                ...
                marks: data.marks ? String(data.marks) : "",
                difficulty: data.difficulty || "EASY"
            });
        } catch (error) {
            alert(error.response?.data || "Failed to fetch question details");
            navigate(-1);
        } finally {
            setFetching(false);
        }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await updateQuestion(quizId, quesId, {
                ...questionData,
            ...
            alert(error.response?.data || "Failed to update question");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (fetching) {
        return (
            <div className="flex h-64 items-center justify-center">
                <p className="text-sm text-slate-500">Loading question details...</p>
            </div>
        );
    }
        ...
        <main className="space-y-4">
            <header className="flex flex-col gap-3">
                <div>
                    ...
                    <p className="text-slate-500">Edit existing question details.</p>
                </div>
            </header>

            <section className="max-w-xl rounded-2xl border bg-white p-5 shadow-sm">
                <h2 className="text-lg font-extrabold">Edit Question</h2>
                <p className="mt-1 text-xs text-gray-400">Update question fields below</p>
